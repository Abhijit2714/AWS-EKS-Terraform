...
data "aws_secretsmanager_secret" "public_key_secret" {
  name = "Public_KeY_for_Bastion"
}

data "aws_secretsmanager_secret_version" "public_key_version" {
  secret_id = data.aws_secretsmanager_secret.public_key_secret.id
}

resource "aws_key_pair" "ssh-key-ec2-instance" {
  key_name   = var.ssh-keyname
  public_key = data.aws_secretsmanager_secret_version.public_key_version.secret_string
}
